...
type State = { tick:number; nextTickAt:number; width:number; height:number; tiles:Tile[][]; players:Player[]; bombs:Bomb[]; flames:{x:number;y:number}[] };
...
export default function Home() {
  ...
  return <main>
    ...
    <section className="gameShell">
      ...
      {game ? <div className="board" style={{gridTemplateColumns:`repeat(${game.width},1fr)`,aspectRatio:`${game.width}/${game.height}`}}>
        {game.tiles.flatMap((row,y)=>row.map((tile,x)=>{
          ...
          return <div className={`tile ${tile}`} key={`${x},${y}`}>
            {tile==="crate"&&<span className="box">×</span>}
            {bomb&&<span className="bomb">●<i>{bomb.fuse}</i></span>}
            {people.map(p=><span key={p.id} className={`fighter ${p.id===myId?"me":p.isAI?"ai":"rival"}`} title={p.id}>{p.isAI?"AI":"◉"}</span>)}
      ...
      <div className="legend"><span><i className="gray"/>고정 벽</span><span><i className="yellow"/>파괴 후 8초 뒤 복구</span><span><i className="cyan"/>나</span><span><i className="coral"/>AI / 다른 플레이어</span><button onClick={()=>setSound(v=>!v)}>{sound?"♪ 똑딱 소리 켜짐":"× 소리 꺼짐"}</button></div>
    </section>
  </main>;
}

...
export default function Home() {
  ...
  const applyWorld=useCallback((s:any)=>{const token=worldToken.current,me=s.players[token];if(!me)return;if(!cameraRef.current)cameraRef.current={x:me.x-Math.floor(W/2),y:me.y-Math.floor(H/2)};let{ x:ox,y:oy}=cameraRef.current,sx=me.x-ox,sy=me.y-oy;if(sx<2)ox=me.x-2;if(sx>W-3)ox=me.x-(W-3);if(sy<2)oy=me.y-2;if(sy>H-3)oy=me.y-(H-3);cameraRef.current={x:ox,y:oy};sx=me.x-ox;sy=me.y-oy;const destroyed=s.destroyed||{},clear=s.clear||{};const tiles=Array.from({length:H},(_,y)=>Array.from({length:W},(_,x)=>{const wx=ox+x,wy=oy+y;if(wx%2===0&&wy%2===0)return"wall" as Tile;if(clear[`${wx},${wy}`]||destroyed[`${wx},${wy}`])return"floor" as Tile;return hash(wx,wy,s.seed)%100<58?"crate" as Tile:"floor" as Tile}));const rivals=Object.entries(s.players).filter(([id])=>id!==token).map(([,p]:any)=>({x:p.x-ox,y:p.y-oy}));const bombs=s.bombs.map((b:any)=>({...b,x:b.x-ox,y:b.y-oy,owner:b.owner===token?"p":"b"}));const flames=s.flames.map((f:any)=>({x:f.x-ox,y:f.y-oy}));smooth(()=>setG(old=>({...old,tiles,player:{x:sx,y:sy},rivals,bombs,flames,power:me.power,range:me.range,status:"playing"})))},[]);
  ...
  const randomMatch=()=>{worldToken.current=crypto.randomUUID();cameraRef.current=null;onlineRef.current={room:"WORLD",role:"p1"};setRoom("WORLD");setRole("p1");setLobby(false);setEndless(true);setMatching(false);setWaiting(false)};

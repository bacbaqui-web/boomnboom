...
- 아이템은 AI가 사망한 authoritative 위치에만 생성하며 상자 파괴, 월드 생성과
  사람 사망은 아이템을 만들지 않는다.
- AI 사망 아이템을 사람이 획득하지 않으면 생성 10초 뒤 server tick에서 제거한다.
- 폭발로 파괴된 상자는 12초 뒤 복구한다. 복구 3초 전에 `crate_warning` 장판을
  표시하되 살아 있는 플레이어의 9×9 범위 안에서는 새 장판 생성을 미룬다.
- 예고 장판이 이미 생긴 뒤 플레이어가 접근해도 예정된 복구 tick은 바꾸지 않는다.
...
- 사람과 AI의 같은 월드 참여
- 칸 단위 충돌과 폭탄 설치
- AI 사망 위치에만 생기고 10초 뒤 만료되는 폭탄 수, 방어막, 화력, 이동속도 아이템
- 화면 밖 적 방향 표시
- 상자 파괴 12초 뒤 복구와 3초 전 예고 장판
...
- 월드 생성, 음수 좌표, 청크 경계, 충돌, 폭발, 상자 예고·복구와 protocol 순서를
  자동 테스트한다.
